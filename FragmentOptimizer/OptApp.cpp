...
COptApp::COptApp(void)
    : resolution_(8), interval_(50), num_(0), weight_(1.0f), length_(3.0f),
      max_iteration_(5), max_inner_iteration_(10), ctr_filename_("output.ctr"),
      sample_filename_("sample.pcd"), pose_filename_("pose.log"),
      init_ctr_file_(""), dir_prefix_(""), sample_num_(-1),
...
void COptApp::InitIPose() {
    ...
    Eigen::Matrix4d leftbase =
        basepose * rgbd_traj_.data_[0].transformation_.inverse();
    ipose_.resize(num_);
    pose_.resize(num_);
...
#pragma omp parallel for num_threads(8) schedule(dynamic)
    for (int i = 0; i < num_; i++) {
        ...
        if (boost::filesystem::exists(std::string(filename))) {
            ...
            sprintf(filename, "%scloud_bin_xyzn_%d.xyzn", dir_prefix_.c_str(),
                    ii);
            if (boost::filesystem::exists(std::string(filename))) {
...
void COptApp::OptimizeNonrigid() {
    ...
    PCL_INFO("Parameters: weight %.5f, resolution %d, piece number %d, max "
             "iteration %d\n",
...
#pragma omp critical
        ...
        for (int m = 0; m < max_inner_iteration_; m++) {
                ...
                for (int i = 0; i <= resolution_; i++) {
                    for (int j = 0; j <= resolution_; j++) {
                        for (int k = 0; k <= resolution_; k++) {
                            ...
                            std::vector<int> idxx;
                            if (i > 0) {
                                idxx.push_back(GetIndex(i - 1, j, k) * 3 +
                                               l * nper_);
                            }
                            if (i < resolution_) {
                                idxx.push_back(GetIndex(i + 1, j, k) * 3 +
                            ...
                            if (j > 0) {
                                idxx.push_back(GetIndex(i, j - 1, k) * 3 +
                                               l * nper_);
                            }
                            if (j < resolution_) {
                                idxx.push_back(GetIndex(i, j + 1, k) * 3 +
                            ...
                            if (k > 0) {
                                idxx.push_back(GetIndex(i, j, k - 1) * 3 +
                                               l * nper_);
                            }
                            if (k < resolution_) {
                                idxx.push_back(GetIndex(i, j, k + 1) * 3 +
            ...
            PCL_WARN("Iteration #%d:%d (%d:%d) : score is %.4f\n", itr + 1,
                     m + 1, max_iteration_, max_inner_iteration_, score);
...
#pragma omp parallel for num_threads(8) schedule(dynamic)
        for (int l = 0; l < (int)corres_.size(); l++) {
            ...
            for (int k = 0; k < (int)corres_[l].corres_.size(); k++) {
                ...
                val[0] = Eigen::Vector4d(0, -ppi(2), ppi(1), 1).dot(npi) +
                         Eigen::Vector4d(0, -npi(2), npi(1), 0).dot(ppi - ppj);
                val[1] = Eigen::Vector4d(ppi(2), 0, -ppi(0), 1).dot(npi) +
                         Eigen::Vector4d(npi(2), 0, -npi(0), 0).dot(ppi - ppj);
                val[2] = Eigen::Vector4d(-ppi(1), ppi(0), 0, 1).dot(npi) +
                         Eigen::Vector4d(-npi(1), npi(0), 0, 0).dot(ppi - ppj);
                val[3] = npi(0);
                val[4] = npi(1);
                val[5] = npi(2);
...
#pragma omp critical
        ...
        for (int l = 0; l < num_; l++) {
            ...
            aff_mat.linear() =
                (Eigen::Matrix3d)Eigen::AngleAxisd(result(l * 6 + 2),
...
#pragma omp critical
            ...
            for (int i = 0; i <= resolution_; i++) {
                for (int j = 0; j <= resolution_; j++) {
                    for (int k = 0; k <= resolution_; k++) {
                        ...
                        Eigen::Matrix3d R;
                        if (i == resolution_ / 2 && j == resolution_ / 2 &&
                            k == 0) {
            ...
            for (int l = 0; l < num_; l++) {
                ...
                aff_mat.linear() =
                    ...
                    Eigen::AngleAxisd(result(l * 6 + 1),
                                      Eigen::Vector3d::UnitY()) *
                    Eigen::AngleAxisd(result(l * 6 + 0),
    ...
    if (init_ctr_file_.length() > 1) {
        ...
        if (f != NULL) {
            ...
            for (int i = 0; i < num_ * nper_ / 3; i++) {
                fgets(buffer, 1024, f);
                sscanf(buffer, "%lf %lf %lf", &ctr(i * 3), &ctr(i * 3 + 1),
                       &ctr(i * 3 + 2));
            }
            fclose(f);
        }
        ...
        for (int l = 0; l < num_; l++) {
            for (int i = 0; i <= resolution_; i++) {
                for (int j = 0; j <= resolution_; j++) {
                    for (int k = 0; k <= resolution_; k++) {
                        Eigen::Vector4d pos(i * unit_length_, j * unit_length_,
                                            k * unit_length_, 1);
    ...
    if (init_ctr_file_.length() > 1) {
        ...
        if (f != NULL) {
            ...
            for (int i = 0; i < nper_ / 3; i++) {
                fgets(buffer, 1024, f);
                sscanf(buffer, "%lf %lf %lf", &thisCtr(num_ * 6 + i * 3),
...
void COptApp::Pose2Ctr(std::vector<Eigen::Matrix4d> &pose,
                       Eigen::VectorXd &ctr) {
    for (int l = 0; l < num_; l++) {
        for (int i = 0; i <= resolution_; i++) {
            for (int j = 0; j <= resolution_; j++) {
...
void COptApp::ExpandCtr(const Eigen::VectorXd &ctr,
                        Eigen::VectorXd &expand_ctr) {
    expand_ctr.setZero();
    for (int l = 0; l < num_; l++) {
        for (int i = 0; i < nper_ / 3; i++) {
            Eigen::Vector4d pos =
                pose_[l] * Eigen::Vector4d(ctr(num_ * 6 + i * 3 + 0),
                                           ctr(num_ * 6 + i * 3 + 1),
                                           ctr(num_ * 6 + i * 3 + 2), 1);
...
Eigen::Matrix3d COptApp::GetRotation(const int idx,
                                     const std::vector<int> &idxx,
                                     const Eigen::VectorXd &ictr,
                                     const Eigen::VectorXd &ctr) {
    int n = (int)idxx.size();
...
void COptApp::SavePoints(const Eigen::VectorXd &ctr, std::string filename) {
    ...
    for (int l = 0; l < num_; l++) {
        pointclouds_[l].UpdateAllNormal(ctr);
        for (int i = 0; i < (int)pointclouds_[l].points_.size();
             i += sample_num_) {
            Eigen::Vector3d pos =
                pointclouds_[l].UpdatePoint(ctr, pointclouds_[l].points_[i]);
            Eigen::Vector3d norm(pointclouds_[l].points_[i].n_[0],
