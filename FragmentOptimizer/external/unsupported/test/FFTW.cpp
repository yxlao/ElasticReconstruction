...
template <typename T> std::complex<T> RandomCpx() {
    return std::complex<T>((T)(rand() / (T)RAND_MAX - .5),
                           (T)(rand() / (T)RAND_MAX - .5));
}
...
template <typename T> complex<long double> promote(complex<T> x) {
    return complex<long double>(x.real(), x.imag());
}
...
long double fft_rmse(const VT1 &fftbuf, const VT2 &timebuf) {
    ...
    for (size_t k0 = 0; k0 < (size_t)fftbuf.size(); ++k0) {
        ...
        for (size_t k1 = 0; k1 < (size_t)timebuf.size(); ++k1) {
            acc +=
                promote(timebuf[k1]) * exp(complex<long double>(0, k1 * phinc));
        }
        totalpower += norm(acc);
...
template <int Container, typename T> void test_complex_generic(int nfft) {
    ...
    ComplexVector buf3;
    for (int k = 0; k < nfft; ++k)
        inbuf[k] = Complex((T)(rand() / (double)RAND_MAX - .5),
...
void test_FFTW() {
    ...
    // CALL_SUBTEST( ( test_complex2d<float,4,8> () ) ); CALL_SUBTEST( (
    // test_complex2d<double,4,8> () ) ); CALL_SUBTEST( ( test_complex2d<long
    // double,4,8> () ) );
