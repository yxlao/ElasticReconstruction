...
void evalSolverSugarFunction(const POLYNOMIAL &pols, const ROOTS &roots,
    ...
    if (aux_evalSolver<Deg, POLYNOMIAL, PolynomialSolverType>(pols, psolve)) {
        ...
        for (size_t i = 0; i < calc_realRoots.size(); ++i) {
            ...
            for (size_t j = 0; j < calc_realRoots.size() && !found; ++j) {
                if (internal::isApprox(calc_realRoots[i], real_roots[j]),
                    psPrec) {
                    found = true;
                }
            }
        ...
        VERIFY(internal::isApprox(roots.array().abs().maxCoeff(),
                                  internal::abs(psolve.greatestRoot()),
                                  psPrec));

        // Test smallestRoot
        VERIFY(internal::isApprox(roots.array().abs().minCoeff(),
                                  internal::abs(psolve.smallestRoot()),
                                  psPrec));
        ...
        if (hasRealRoot) {
            VERIFY(internal::isApprox(real_roots.array().abs().maxCoeff(),
        ...
        VERIFY(hasRealRoot == (real_roots.size() > 0));
        if (hasRealRoot) {
            VERIFY(internal::isApprox(real_roots.array().abs().minCoeff(),
                                      internal::abs(r), psPrec));
        }
        ...
        if (hasRealRoot) {
            VERIFY(
                internal::isApprox(real_roots.array().maxCoeff(), r, psPrec));
        }
        ...
        if (hasRealRoot) {
            VERIFY(
                internal::isApprox(real_roots.array().minCoeff(), r, psPrec));
        }
    }
}
...
template <typename _Scalar, int _Deg> void polynomialsolver(int deg) {
    ...
    evalSolverSugarFunction<_Deg>(
        pols,
        realRoots
            .template cast<std::complex<typename NumTraits<_Scalar>::Real>>(),
        realRoots);
}
