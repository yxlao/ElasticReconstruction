...
#include "BuildCorrespondence/Helper.h"
...
class CCorresApp {
  ...
  public:
    void ToggleSaveXYZN() { save_xyzn_ = true; }
    double NormalDot(pcl::PointXYZRGBNormal &p0, pcl::PointXYZRGBNormal &p1) {
        return (p0.normal_x * p1.normal_x + p0.normal_y * p1.normal_y +
                p0.normal_z * p1.normal_z);
    }
    int GetReduxIndex(int i, int j) { return i + j * num_; }
    double GetVolumeOverlapRatio(Eigen::Matrix4d &trans) {
        ...
        for (int i = 0; i < res; i++) {
            for (int j = 0; j < res; j++) {
                for (int k = 0; k < res; k++) {
                    Eigen::Vector4d pos((i + 0.5) * ul, (j + 0.5) * ul,
                                        (k + 0.5) * ul, 1);
                    Eigen::Vector4d ppos = trans * pos;
                    if (ppos(0) >= 0 && ppos(0) <= length_ && ppos(1) >= 0 &&
                        ppos(1) <= length_ && ppos(2) >= 0 &&
                        ppos(2) <= length_) {
                        s++;
                    }
                }
