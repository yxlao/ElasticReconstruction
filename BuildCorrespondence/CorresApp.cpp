...
CCorresApp::CCorresApp(void)
    : save_xyzn_(false), save_corres_(true), dist_thresh_(0.015),
      normal_thresh_(0.8660), registration_(false), output_information_(false),
      reg_dist_(0.03), reg_ratio_(0.25), reg_num_(40000), redux_(false),
      num_(0), bbox_length_(-1.0), length_(3.0), interval_(50) {}
...
void CCorresApp::LoadData(std::string filename, int num) {
    ...
    if (num > 0) {
        ...
        Eigen::Matrix4d leftbase =
            basepose * temp.data_[0].transformation_.inverse();
        std::vector<Eigen::Matrix4d> ipose;
        ipose.resize(num_);

        for (int i = 0; i < num_; i++) {
            ipose[i] = leftbase * temp.data_[i * interval_].transformation_ *
                       baseinverse;
        }

        for (int i = 0; i < num_ - 1; i++) {
            corres_traj_.data_.push_back(FramedTransformation(
                i, i + 1, num_, ipose[i].inverse() * ipose[i + 1]));
            for (int j = i + 2; j < num; j++) {
                Eigen::Matrix4d trans = ipose[i].inverse() * ipose[j];
                if (GetVolumeOverlapRatio(trans) > 0.3) {
                    corres_traj_.data_.push_back(
                        FramedTransformation(i, j, num_, trans));
                }
            }
        }
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < num_; i++) {
        char fn[1024];
        memset(fn, 0, 1024);
        pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr rawpcd(
...
void CCorresApp::FindCorrespondence() {
    if (output_information_) {
        ...
        for (int i = 0; i < (int)corres_traj_.data_.size(); i++) {
            corres_info_.data_.push_back(FramedInformation(
                corres_traj_.data_[i].id1_, corres_traj_.data_[i].id2_,
                corres_traj_.data_[i].frame_, InformationMatrix::Zero()));
        }
    }
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < (int)corres_traj_.data_.size(); i++) {
        ...
        PCL_INFO("Processing pair <%d, %d>\n", corres_traj_.data_[i].id1_,
                 corres_traj_.data_[i].id2_);

        corres.clear();
        pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr pcd0 =
            pointclouds_[corres_traj_.data_[i].id1_];
        pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr pcd1 =
            pointclouds_[corres_traj_.data_[i].id2_];

        pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr transformed(
            new pcl::PointCloud<pcl::PointXYZRGBNormal>);
        pcl::transformPointCloudWithNormals(
            *pcd1, *transformed, corres_traj_.data_[i].transformation_);
        ...
        for (int k = 0; k < (int)transformed->size(); k++) {
            if (tree.nearestKSearch(transformed->points[k], K,
                                    pointIdxNKNSearch,
                                    pointNKNSquaredDistance) > 0) {
                if (pointNKNSquaredDistance[0] < dist_thresh_ * dist_thresh_ &&
                    NormalDot(pcd0->points[pointIdxNKNSearch[0]],
                              transformed->points[k]) > normal_thresh_) {

                    corres.push_back(
                        CorrespondencePair(pointIdxNKNSearch[0], k));
                }
            }
        }
        ...
        PCL_INFO("    <%d, %d> : Corresponce number is %d, ratio is %.2f(%d)\n",
                 corres_traj_.data_[i].id1_, corres_traj_.data_[i].id2_,
                 corres.size(),
                 (double)corres.size() / (double)corres_traj_.data_[i].frame_,
                 corres_traj_.data_[i].frame_);
        if ((double)corres.size() / (double)corres_traj_.data_[i].frame_ <
            0.5) {
        ...
        if (save_corres_) {
            char fn[1024];
            memset(fn, 0, 1024);
            sprintf(fn, "%scorres_%d_%d.txt", m_pDirName,
                    corres_traj_.data_[i].id1_, corres_traj_.data_[i].id2_);
            FILE *f = fopen(fn, "w");
...
#pragma omp atomic
        ...
        PCL_INFO(
            "    <%d, %d> : %d inliers with ratio %.2f(%d) and %.2f(%d) ... ",
        ...
        if (redux_) {
            ...
            if (it != redux_map_.end()) {
                corres_traj_.data_[i].transformation_ =
                    redux_traj_.data_[it->second].transformation_;
#pragma omp atomic
                nprocessed++;
                PCL_WARN("%d / %d\n", nprocessed, corres_traj_.data_.size());
                continue;
        ...
        icp.align(*transformed,
                  corres_traj_.data_[i].transformation_.cast<float>());
...
void CCorresApp::Redux(std::string filename) {
    ...
    for (int i = 0; i < (int)redux_traj_.data_.size(); i++) {
        redux_map_.insert(IntPair(
            GetReduxIndex(redux_traj_.data_[i].id1_, redux_traj_.data_[i].id2_),
            i));
    }

    PCL_WARN("%d out of %d pairs are redux pairs.\n", redux_traj_.data_.size(),
             corres_traj_.data_.size());
}
