...
CIntegrateApp::CIntegrateApp(pcl::Grabber &source, bool use_device)
    : cols_(640), rows_(480), volume_(cols_, rows_), capture_(source),
      use_device_(use_device), exit_(false), registration_(false), time_ms_(0),
      frame_id_(0), traj_filename_(""), pose_filename_(""), seg_filename_(""),
      camera_filename_(""), ctr_filename_(""), pcd_filename_("world.pcd"),
      ctr_num_(0), ctr_resolution_(8), ctr_interval_(50), ctr_length_(3.0),
...
void CIntegrateApp::Init() {
    ...
    if (ctr_num_ > 0 && boost::filesystem::exists(ctr_filename_) &&
        boost::filesystem::exists(seg_filename_)) {
        ...
        if (boost::filesystem::exists(pose_filename_)) {
            ...
            for (int i = 0; i < (int)pose_traj_.data_.size(); i++) {
                for (int j = 0; j < ctr_interval_; j++) {
                    ...
                    traj_.data_.push_back(FramedTransformation(
                        idx, idx, idx + 1,
                        pose_traj_.data_[i].transformation_ *
                            seg_traj_.data_[idx].transformation_));
                }
            }
            PCL_WARN(
...
void CIntegrateApp::StartMainLoop(bool triggered_capture) {
    ...
    if (!triggered_capture) {
        call_back_func =
            boost::bind(&CIntegrateApp::source_cb2, this, _1, _2, _3);
    } else {
        call_back_func =
            boost::bind(&CIntegrateApp::source_cb2_trigger, this, _1, _2, _3);
    }
...

    {
        ...
        while (!exit_) {
            ...
            has_data = data_ready_cond_.timed_wait(
                lock, boost::posix_time::millisec(300));
...
#ifdef IMAGE_VIEWER
                viewer_depth_.showShortImage(&depth_[0], cols_, rows_, 0, 3000,
                                             true);
...
// Capture functions
//////////////////////////////////////////////
void CIntegrateApp::source_cb2(
    const boost::shared_ptr<openni_wrapper::Image> &image_wrapper,
    const boost::shared_ptr<openni_wrapper::DepthImage> &depth_wrapper, float) {
    {
        ...
        if (depth_wrapper->getWidth() != image_wrapper->getWidth() ||
            depth_wrapper->getHeight() != image_wrapper->getHeight()) {
            PCL_ERROR(
                "Resolution of depth stream and image stream must match.");
            return;
        }

        if (depth_wrapper->getWidth() != cols_ ||
            depth_wrapper->getHeight() != rows_) {
...
void CIntegrateApp::source_cb2_trigger(
    const boost::shared_ptr<openni_wrapper::Image> &image_wrapper,
    const boost::shared_ptr<openni_wrapper::DepthImage> &depth_wrapper, float) {
    {
...
void CIntegrateApp::Execute(bool has_data) {
    ...
    volume_.Integrate(depth_, scaled_depth_,
                      traj_.data_[frame_id_ - 1].transformation_);
}
...
void CIntegrateApp::Reproject() {
    ...
    Eigen::Matrix4d TiT0Ai_adj =
        traj_.data_[frame_id_ - 1].transformation_.inverse() *
        traj_.data_[0].transformation_ *
        seg_traj_.data_[0].transformation_.inverse();
        ...
        for (int u = 0; u < cols_; u += 1) {
            ...
            if (volume_.UVD2XYZ(u, v, d, x, y, z)) {
                Eigen::Vector4d dummy =
                    seg_traj_.data_[frame_id_ - 1].transformation_ *
                    Eigen::Vector4d(x, y, z, 1);
                Coordinate coo;
                Eigen::Vector3f pos;

                if (grids_[chunk].GetCoordinate(
                        Eigen::Vector3f(dummy(0), dummy(1), dummy(2)),
                        coo)) { // in the box, thus has the right coo
                    grids_[chunk].GetPosition(coo, pos);
                    Eigen::Vector4d reproj_pos =
                        TiT0Ai_adj *
                        Eigen::Vector4d(pos(0), pos(1), pos(2), 1.0);

                    if (volume_.XYZ2UVD(reproj_pos(0), reproj_pos(1),
                                        reproj_pos(2), uu, vv, dd)) {
