...
TSDFVolume::TSDFVolume(int cols, int rows)
    : cols_(cols), rows_(rows), unit_length_(3.0 / 512.0), tsdf_trunc_(0.03) {}

TSDFVolume::~TSDFVolume(void) {}

void TSDFVolume::ScaleDepth(std::vector<unsigned short> &depth,
...
void TSDFVolume::Integrate(std::vector<unsigned short> &depth,
                           std::vector<float> &scaled,
    ...
    for (int v = 0; v < rows_; v += 1) {
        for (int u = 0; u < cols_; u += 1) {
            ...
            if (UVD2XYZ(u, v, d, x, y, z)) {
                Eigen::Vector4d vv =
                    transformation * Eigen::Vector4d(x, y, z, 1);
                ...
                if (touched_unit.find(key) == touched_unit.end()) {
                    ...
                    if (data_.find(key) == data_.end()) {
                        data_[key] = TSDFVolumeUnit::Ptr(
                            new TSDFVolumeUnit(64, xi, yi, zi));
                    }
                    IntegrateVolumeUnit(scaled, transformation.cast<float>(),
                                        trans_inv.cast<float>(), data_[key],
                                        I2F(xi), I2F(yi), I2F(zi));
                }
            }
        }
    }
...
void TSDFVolume::IntegrateVolumeUnit(std::vector<float> &scaled,
                                     const Eigen::Matrix4f &trans,
                                     const Eigen::Matrix4f &trans_inv,
                                     TSDFVolumeUnit::Ptr volume, float x_shift,
                                     float y_shift, float z_shift) {
#pragma omp parallel for num_threads(8)
    for (int i = 0; i < volume->resolution_; i++) {
        for (int j = 0; j < volume->resolution_; j++) {
            for (int k = 0; k < volume->resolution_; k++) {
                Eigen::Vector4f gridv(i * unit_length_ + x_shift,
                                      j * unit_length_ + y_shift,
                                      k * unit_length_ + z_shift, 1);
                Eigen::Vector4f tgv = trans_inv * gridv;
                if (tgv(2) > 0) {
                    int coox =
                        round(tgv(0) * camera_.fx_ / tgv(2) + camera_.cx_);
                    int cooy =
                        round(tgv(1) * camera_.fy_ / tgv(2) + camera_.cy_);
                    if (coox >= 0 && coox < cols_ && cooy >= 0 &&
                        cooy < rows_) {
                        ...
                        if (dp_scaled > 0.001f) {
                            ...
                            float sdf =
                                dp_scaled - sqrtf(rx * rx + ry * ry + rz * rz);
                            if (sdf >= -tsdf_trunc_) {
                                float tsdf =
                                    std::min<float>(1.0f, sdf / tsdf_trunc_);
                                // need to change weight
                                float w = 1.0f;

                                int lll = (i * volume->resolution_ + j) *
                                              volume->resolution_ +
                                          k;
                                volume->sdf_[lll] =
                                    (volume->sdf_[lll] * volume->weight_[lll] +
                                     w * tsdf) /
