...
#include "GlobalRegistration/PolyRejector.h"
...
class RansacCurvature : public Registration<PointSource, PointTarget> {
  public:
    ...
    typedef boost::shared_ptr<
        RansacCurvature<PointSource, PointTarget, FeatureT>>
        Ptr;
    typedef boost::shared_ptr<
        const RansacCurvature<PointSource, PointTarget, FeatureT>>
        ConstPtr;
    ...
    typedef typename CorrespondenceRejectorPoly::ConstPtr
        CorrespondenceRejectorPolyConstPtr;

    /** \brief Constructor */
    RansacCurvature()
        : input_features_(), target_features_(), nr_samples_(3),
          k_correspondences_(2), feature_tree_(new pcl::KdTreeFLANN<FeatureT>),
          correspondence_rejector_poly_(new CorrespondenceRejectorPoly),
          inlier_fraction_(0.0f), inlier_number_(1000000), angle_diff_(M_PI) {
        reg_name_ = "RansacCurvature";
        correspondence_rejector_poly_->setSimilarityThreshold(0.6f);
        max_iterations_ = 5000;
        transformation_estimation_.reset(
            new pcl::registration::TransformationEstimationSVD<PointSource,
                                                               PointTarget>);
    };
    ...
    /** \brief Provide a boost shared pointer to the source point cloud's
     * feature descriptors \param features the source point cloud's features
     */
    ...
    /** \brief Provide a boost shared pointer to the target point cloud's
     * feature descriptors \param features the target point cloud's features
     */
     ...
     */
    inline void setNumberOfSamples(int nr_samples) { nr_samples_ = nr_samples; }

    /** \brief Get the number of samples to use during each iteration, as set by
     * the user */
    inline int getNumberOfSamples() const { return (nr_samples_); }

    /** \brief Set the number of neighbors to use when selecting a random
     * feature correspondence.  A higher value will add more randomness to the
     * feature matching. \param k the number of neighbors to use when selecting
     * a random feature correspondence.
     */
    inline void setCorrespondenceRandomness(int k) { k_correspondences_ = k; }
     ...
     * underlying polygonal correspondence rejector object, where 1 is a perfect
     * match \param similarity_threshold edge length similarity threshold
     */
    inline void setSimilarityThreshold(float similarity_threshold) {
        correspondence_rejector_poly_->setSimilarityThreshold(
            similarity_threshold);
    }

    /** \brief Get the similarity threshold between edge lengths of the
     * underlying polygonal correspondence rejector object, \return edge length
    ...
    inline void setInlierNumber(int inlier_number) {
     ...
     */
    inline float getInlierFraction() const { return inlier_fraction_; }
    ...
    inline void setAngleDiff(float angle_diff) { angle_diff_ = angle_diff; }

    inline bool thresholdNormal(const std::vector<int> &source_indices,
                                const std::vector<int> &target_indices,
                                const Eigen::Matrix4f &transformation) {
        for (int i = 0; i < (int)source_indices.size(); i++) {
            Eigen::Vector4f ns((*input_)[source_indices[i]].normal_x,
                               (*input_)[source_indices[i]].normal_y,
                               (*input_)[source_indices[i]].normal_z, 0);
            Eigen::Vector4f nt((*target_)[target_indices[i]].normal_x,
                               (*target_)[target_indices[i]].normal_y,
                               (*target_)[target_indices[i]].normal_z, 0);
    ...
    void getDenseCorrespondences(std::vector<int> &corres,
  ...
  protected:
    ...
    /** \brief Select \a nr_samples sample points from cloud while making sure
     * that their pairwise distances are greater than a user-defined minimum
     * distance, \a min_sample_distance. \param cloud the input point cloud
     * \param nr_samples the number of samples to select
     * \param sample_indices the resulting sample indices
     */
    void selectSamples(const PointCloudSource &cloud, int nr_samples,
                       std::vector<int> &sample_indices);

    /** \brief For each of the sample points, find a list of points in the
     * target cloud whose features are similar to the sample points' features.
     * From these, select one randomly which will be considered that sample
     * point's correspondence. \param sample_indices the indices of each sample
     * point \param similar_features correspondence cache, which is used to
     * read/write already computed correspondences \param corresponding_indices
     * the resulting indices of each sample's corresponding point in the target
     * cloud
     */
    void findSimilarFeatures(const std::vector<int> &sample_indices,
     ...
     *   - Inliers: the number of transformed points which are closer than
     * threshold to NN
     ...
     */
    void getFitness(std::vector<int> &inliers, std::vector<int> &inliers_target,
                    float &fitness_score);
    ...
    /** \brief The fraction [0,1] of inlier points required for accepting a
     * transformation */
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointSource, typename PointTarget, typename FeatureT>
void RansacCurvature<PointSource, PointTarget, FeatureT>::setSourceFeatures(
    const FeatureCloudConstPtr &features) {
    if (features == NULL || features->empty()) {
        PCL_ERROR("[pcl::%s::setSourceFeatures] Invalid or empty point cloud "
                  "dataset given!\n",
                  getClassName().c_str());
        return;
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointSource, typename PointTarget, typename FeatureT>
void RansacCurvature<PointSource, PointTarget, FeatureT>::setTargetFeatures(
    const FeatureCloudConstPtr &features) {
    if (features == NULL || features->empty()) {
        PCL_ERROR("[pcl::%s::setTargetFeatures] Invalid or empty point cloud "
                  "dataset given!\n",
                  getClassName().c_str());
        return;
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointSource, typename PointTarget, typename FeatureT>
void RansacCurvature<PointSource, PointTarget, FeatureT>::selectSamples(
    ...
    if (nr_samples > static_cast<int>(cloud.points.size())) {
        ...
        PCL_ERROR("The number of samples (%d) must not be greater than the "
                  "number of points (%lu)!\n",
    ...
    for (int i = 0; i < nr_samples; i++) {
        ...
        sample_indices[i] =
            getRandomIndex(static_cast<int>(cloud.points.size()) - i);

        // Run trough list of numbers, starting at the lowest, to avoid
        // duplicates
        for (int j = 0; j < i; j++) {
            // Move value up if it is higher than previous selections to ensure
            // true randomness
            if (sample_indices[i] >= sample_indices[j]) {
                sample_indices[i]++;
            } else {
                // The new number is lower, place it at the correct point and
                // break for a sorted list
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointSource, typename PointTarget, typename FeatureT>
void RansacCurvature<PointSource, PointTarget, FeatureT>::findSimilarFeatures(
    ...
    for (size_t i = 0; i < sample_indices.size(); ++i) {
        ...
        // Find the k nearest feature neighbors to the sampled input feature if
        // they are not in the cache already
        if (similar_features[idx].empty())
            feature_tree_->nearestKSearch(*input_features_, idx,
                                          k_correspondences_,
...
template <typename PointSource, typename PointTarget, typename FeatureT>
void RansacCurvature<PointSource, PointTarget, FeatureT>::
    getDenseCorrespondences(std::vector<int> &corres,
                            std::vector<float> &dis2) {
    // Allocate results
    ...
    for (size_t i = 0; i < input_features_->size(); ++i) {
        // Find the k nearest feature neighbors to the sampled input feature if
        // they are not in the cache already
        feature_tree_->nearestKSearch(*input_features_, i, 1, indices,
                                      nn_distances);
        corres[i] = indices[0];
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointSource, typename PointTarget, typename FeatureT>
void RansacCurvature<PointSource, PointTarget, FeatureT>::computeTransformation(
    PointCloudSource &output, const Eigen::Matrix4f &guess) {
    // Some sanity checks first
    if (!input_features_) {
        PCL_ERROR("[pcl::%s::computeTransformation] ", getClassName().c_str());
        PCL_ERROR("No source features were given! Call setSourceFeatures "
                  "before aligning.\n");
        return;
    }
    if (!target_features_) {
        PCL_ERROR("[pcl::%s::computeTransformation] ", getClassName().c_str());
        PCL_ERROR("No target features were given! Call setTargetFeatures "
                  "before aligning.\n");
        return;
    }
    ...
    if (input_->size() != input_features_->size()) {
        PCL_ERROR("[pcl::%s::computeTransformation] ", getClassName().c_str());
        PCL_ERROR(
            "The source points and source feature points need to be in a "
            "one-to-one relationship! Current input cloud sizes: %ld vs %ld.\n",
    ...
    if (target_->size() != target_features_->size()) {
        PCL_ERROR("[pcl::%s::computeTransformation] ", getClassName().c_str());
        PCL_ERROR(
            "The target points and target feature points need to be in a "
            "one-to-one relationship! Current input cloud sizes: %ld vs %ld.\n",
    ...
    // Initialize prerejector (similarity threshold already set to default value
    // in constructor)
    ...
    for (int i = 0; i < max_iterations_; ++i) {
        ...
        findSimilarFeatures(sample_indices, similar_features,
                            corresponding_indices);

        // Apply prerejection
        if (!correspondence_rejector_poly_->thresholdPolygon(
                sample_indices, corresponding_indices)) {
            ++num_rejections;
            continue;
        }

        // Estimate the transform from the correspondences, write to
        // transformation_
        if (nr_samples_ > 2) {
            transformation_estimation_->estimateRigidTransformation(
                *input_, sample_indices, *target_, corresponding_indices,
                transformation_);
        } else {
            std::vector<int> idxs;
            idxs.push_back(0);
            ...
            Eigen::Vector3d sn0(temp_input[0].normal_x, temp_input[0].normal_y,
                                temp_input[0].normal_z);
            Eigen::Vector3d sn1(temp_input[1].normal_x, temp_input[1].normal_y,
                                temp_input[1].normal_z);
            Eigen::Vector3d sn = sn0 + sn1;
            sn.normalize();
            Eigen::Vector3d sp0(temp_input[0].x, temp_input[0].y,
                                temp_input[0].z);
            Eigen::Vector3d sp1(temp_input[1].x, temp_input[1].y,
                                temp_input[1].z);
            ...
            Eigen::Vector3d tn0(temp_target[0].normal_x,
                                temp_target[0].normal_y,
                                temp_target[0].normal_z);
            Eigen::Vector3d tn1(temp_target[1].normal_x,
                                temp_target[1].normal_y,
                                temp_target[1].normal_z);
            Eigen::Vector3d tn = tn0 + tn1;
            tn.normalize();
            Eigen::Vector3d tp0(temp_target[0].x, temp_target[0].y,
                                temp_target[0].z);
            Eigen::Vector3d tp1(temp_target[1].x, temp_target[1].y,
                                temp_target[1].z);
            ...
            transformation_estimation_->estimateRigidTransformation(
                temp_input, idxs, temp_target, idxs, transformation_);

            if (std::isnan(transformation_(0, 0))) {
        ...
        if (!thresholdNormal(sample_indices, corresponding_indices,
                             transformation_)) {
            ++num_normal_rejections;
            continue;
        }
        ...
        // Transform the input and compute the error (uses input_ and
    ...
    // Debug output
    PCL_DEBUG("[pcl::%s::computeTransformation] Rejected (%i, %i) out of %i "
              "generated pose hypotheses (%i remains).\n",
              getClassName().c_str(), num_rejections, num_normal_rejections,
              max_iterations_,
              max_iterations_ - num_rejections - num_normal_rejections);
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointSource, typename PointTarget, typename FeatureT>
void RansacCurvature<PointSource, PointTarget, FeatureT>::getFitness(
    std::vector<int> &inliers, std::vector<int> &inliers_target,
    float &fitness_score) {
    ...
    for (size_t i = 0; i < input_transformed.points.size(); ++i) {
        ...
        tree_->nearestKSearch(input_transformed.points[i], 1, nn_indices,
                              nn_dists);
...
template <typename PointSource, typename PointTarget, typename FeatureT>
void RansacCurvature<PointSource, PointTarget, FeatureT>::getInformation() {
    ...
    for (int i = 0; i < (int)inliers_.size(); i++) {
        ...
        A << 1, 0, 0, 0, 2 * sz, -2 * sy, 0, 1, 0, -2 * sz, 0, 2 * sx, 0, 0, 1,
            2 * sy, -2 * sx, 0;
        ...
        AA << 1, 0, 0, 0, 2 * tz, -2 * ty, 0, 1, 0, -2 * tz, 0, 2 * tx, 0, 0, 1,
            2 * ty, -2 * tx, 0;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointSource, typename PointTarget, typename FeatureT>
void RansacCurvature<PointSource, PointTarget, FeatureT>::writeAuxData(
    ...
    for (int i = 0; i < rots_.size(); i++) {
        fprintf(f, "%.6f %.6f %.6f %.6f %.6f %.6f %.6f\n", rots_[i].w(),
                rots_[i].x(), rots_[i].y(), rots_[i].z(), trans_[i](0),
                trans_[i](1), trans_[i](2));
    }
...
inline void RansacCurvature<PointSource, PointTarget, Scalar>::align_redux(
    PointCloudSource &output, const Eigen::Matrix4f &guess) {
    if (!initCompute())
        return;
