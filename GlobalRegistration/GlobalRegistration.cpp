        ...
        for (int j = i + 1; j < num; j++) {
            ...
            if (config.estimate_normal_) {
                ...
                for (int i = 0; i < scene->size(); i++) {
                    if (scene->points[i].normal_x *
                                scene_bak->points[i].normal_x +
                            scene->points[i].normal_y *
                                scene_bak->points[i].normal_y +
                            scene->points[i].normal_z *
                                scene_bak->points[i].normal_z <
                        0.0) {
                ...
                for (int i = 0; i < object->size(); i++) {
                    if (object->points[i].normal_x *
                                object_bak->points[i].normal_x +
                            object->points[i].normal_y *
                                object_bak->points[i].normal_y +
                            object->points[i].normal_z *
                                object_bak->points[i].normal_z <
                        0.0) {
            ...
            align.setMaximumIterations(
                config.max_iteration_); // Number of RANSAC iterations
            align.setNumberOfSamples(
                config.num_of_samples_); // Number of points to sample for
                                         // generating/prerejecting a pose
            align.setCorrespondenceRandomness(
                config.correspondence_randomness_); // Number of nearest
                                                    // features to use
            align.setSimilarityThreshold(
                config.edge_similarity_); // Polygonal edge length similarity
                                          // threshold
            align.setMaxCorrespondenceDistance(
                config.max_correspondence_distance_); // Inlier threshold
            align.setInlierFraction(
                config.inlier_fraction_); // Required inlier fraction for
...
#pragma omp critical
                {
                    ...
                    traj.data_.push_back(FramedTransformation(
                        i, j, num, transformation.cast<double>()));
                    info.data_.push_back(
                        FramedInformation(i, j, num, information));
                }
            } else {
...
void create_init_traj() {
    ...
    for (size_t i = 0; i < segment_traj.data_.size(); i++) {
        if (i % fragment == 0 && i > 0) {
            base_mat = init_traj.data_[i - 1].transformation_ *
                       segment_traj.data_[i].transformation_.inverse();
        }
        init_traj.data_.push_back(FramedTransformation(
            i, i, i + 1, base_mat * segment_traj.data_[i].transformation_));
    }
...
void create_pose_traj() {
    ...
    for (size_t i = 0; i < init_traj.data_.size(); i += fragment) {
        pose_traj.data_.push_back(FramedTransformation(
            i / fragment, i / fragment, i / fragment + 1,
            init_traj.data_[i].transformation_ *
                segment_traj.data_[0].transformation_.inverse()));
    }
...
void create_odometry(int num) {
    ...
    for (int i = 1; i < num; i++) {
        odometry_traj.data_.push_back(FramedTransformation(
            i - 1, i, num,
            pose_traj.data_[i - 1].transformation_.inverse() *
                pose_traj.data_[i].transformation_));
        pcl::console::print_highlight("Betweeen fragments %d and %d.\n", i - 1,
                                      i);
        PointCloudT::Ptr object(new PointCloudT);
        PointCloudT::Ptr object_aligned(new PointCloudT);
        ...
        align.setNumberOfSamples(
            config.num_of_samples_); // Number of points to sample for
                                     // generating/prerejecting a pose
        align.setCorrespondenceRandomness(
            config.correspondence_randomness_); // Number of nearest features to
                                                // use
        align.setSimilarityThreshold(
            config.edge_similarity_); // Polygonal edge length similarity
                                      // threshold
        align.setMaxCorrespondenceDistance(
            config.max_correspondence_distance_); // Inlier threshold
        align.setInlierFraction(
            config.inlier_fraction_); // Required inlier fraction for accepting
                                      // a pose hypothesis
        align.setInlierNumber(config.inlier_number_);
        align.setAngleDiff(config.angle_difference_);
        {
            ...
            align.align_redux(
                *object_aligned,
                odometry_traj.data_[i - 1].transformation_.cast<float>());
        }

        align.getInformation();
        odometry_info.data_.push_back(
            FramedInformation(i - 1, i, num, align.information_source_));
    }
...
int main(int argc, char *argv[]) {
    ...
    int num_of_pcds =
        std::count_if(boost::filesystem::directory_iterator(
