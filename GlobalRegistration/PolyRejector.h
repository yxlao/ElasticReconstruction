...
class PolyRejector : public registration::CorrespondenceRejector {
  ...
  public:
    ...
    PolyRejector()
        : iterations_(10000), cardinality_(3), similarity_threshold_(0.75f),
     ...
     * used to compute the correspondence distance. \param[in] cloud a cloud
     * containing XYZ data
     */
    ...
    inline void setInputCloud(const PointCloudSourceConstPtr &cloud) {
        PCL_WARN("[pcl::registration::%s::setInputCloud] setInputCloud is "
                 "deprecated. Please use setInputSource instead.\n",
                 getClassName().c_str());
        input_ = cloud;
    }

    /** \brief Provide a target point cloud dataset (must contain XYZ data!),
     * used to compute the correspondence distance. \param[in] target a cloud
     * containing XYZ data
     */
    ...
    */
     ...
     */
    inline void setCardinality(int cardinality) { cardinality_ = cardinality; }

    /** \brief Get the polygon cardinality
     * \return polygon cardinality
     */
    inline int getCardinality() { return (cardinality_); }
     ...
     */
    inline float getSimilarityThreshold() { return (similarity_threshold_); }
     ...
     * correspondences \param corr all correspondences into \ref input_ and \ref
     * target_ \param idx sampled indices into \b correspondences, must have a
     * size equal to \ref cardinality_ \return true if all edge length ratios
     * are larger than or equal to \ref similarity_threshold_
     */
    inline bool thresholdPolygon(const pcl::Correspondences &corr,
                                 const std::vector<int> &idx) {
        if (cardinality_ == 2) // Special case: when two points are considered,
                               // we only have one edge
        {
            ...
            //	if (!thresholdEdgeLength (corr[ idx[i] ].index_query, corr[
            //idx[(i+1)%cardinality_] ].index_query, 		corr[ idx[i] ].index_match,
            //corr[ idx[(i+1)%cardinality_] ].index_match,
            //		similarity_threshold_squared_))
            //		return (false);
            for (int i = 0; i < cardinality_; ++i)
                for (int j = i + 1; j < cardinality_; j++)
                    if (!thresholdEdgeLength(
                            corr[idx[i]].index_query, corr[idx[j]].index_query,
    ...
    /** \brief Polygonal rejection of a single polygon, indexed by two point
     * index vectors \param source_indices indices of polygon points in \ref
     * input_, must have a size equal to \ref cardinality_ \param target_indices
     * corresponding indices of polygon points in \ref target_, must have a size
     * equal to \ref cardinality_ \return true if all edge length ratios are
     * larger than or equal to \ref similarity_threshold_
     */
    inline bool thresholdPolygon(const std::vector<int> &source_indices,
                                 const std::vector<int> &target_indices) {
        // Convert indices to correspondences and an index vector pointing to
        // each element
  ...
  protected:
    ...
    /** \brief Get k unique random indices in range {0,...,n-1} (sampling
     * without replacement) \note No check is made to ensure that k <= n. \param
     * n upper index range, exclusive \param k number of unique indices to
     * sample \return k unique random indices in range {0,...,n-1}
     */
    ...
    /** \brief Squared Euclidean distance between two points using the members
     * x, y and z \param p1 first point \param p2 second point \return squared
     * Euclidean distance
     */
    ...
    inline bool thresholdEdgeLength(int index_query_1, int index_query_2,
        ...
        const float dist_src = computeSquaredDistance((*input_)[index_query_1],
                                                      (*input_)[index_query_2]);
        // Distance between target points
        const float dist_tgt = computeSquaredDistance(
            (*target_)[index_match_1], (*target_)[index_match_2]);
    ...
    /** \brief Compute a linear histogram. This function is equivalent to the
     * MATLAB function \b histc, with the edges set as follows: <b>
     * lower:(upper-lower)/bins:upper </b> \param data input samples \param
     * lower lower bound of input samples \param upper upper bound of input
     * samples \param bins number of bins in output \return linear histogram
     */
    std::vector<int> computeHistogram(const std::vector<float> &data,
                                      float lower, float upper, int bins);

    /** \brief Find the optimal value for binary histogram thresholding using
     * Otsu's method \param histogram input histogram \return threshold value
     * according to Otsu's criterion
     */
    ...
    /** \brief Squared value if \ref similarity_threshold_, only for internal
     * use */
...
///////////////////////////////////////////////////////////////////////////////////////////
template <typename SourceT, typename TargetT>
void PolyRejector<SourceT, TargetT>::getRemainingCorrespondences(
    const pcl::Correspondences &original_correspondences,
    pcl::Correspondences &remaining_correspondences) {
    ...
    // Check source/target
    if (!input_) {
        PCL_ERROR("[pcl::registration::%s::getRemainingCorrespondences] No "
                  "source was input! Returning all input correspondences.\n",
                  getClassName().c_str());
        return;
    }
    ...
    if (!target_) {
        PCL_ERROR("[pcl::registration::%s::getRemainingCorrespondences] No "
                  "target was input! Returning all input correspondences.\n",
                  getClassName().c_str());
        return;
    }
    ...
    if (cardinality_ >= nr_correspondences) {
        PCL_ERROR("[pcl::registration::%s::getRemainingCorrespondences] Number "
                  "of correspondences smaller than polygon cardinality! "
                  "Returning all input correspondences.\n",
                  getClassName().c_str());
        return;
    }
    ...
    if (similarity_threshold_ < 0.0f || similarity_threshold_ > 1.0f) {
        PCL_ERROR("[pcl::registration::%s::getRemainingCorrespondences] "
                  "Invalid edge length similarity - must be in [0,1]!. "
                  "Returning all input correspondences.\n",
                  getClassName().c_str());
        return;
    }
    ...
    // Number of times a correspondence is sampled and number of times it was
    // accepted
    ...
    // Find the cut point between outliers and inliers using Otsu's thresholding
    // method
...
//////////////////////////////////////////////////////////////////////////////////////////////
template <typename SourceT, typename TargetT>
std::vector<int> PolyRejector<SourceT, TargetT>::computeHistogram(
...
//////////////////////////////////////////////////////////////////////////////////////////////
template <typename SourceT, typename TargetT>
int PolyRejector<SourceT, TargetT>::findThresholdOtsu(
    const std::vector<int> &histogram) {
    // Precision
    ...
    for (int i = 0; i < nbins; ++i) {
        ...
        class_mean1 =
            (class_mean1 + static_cast<double>(i) * prob_i) / class_prob1;

        // Class mean 2: sum of probabilities from i+1 to nbins-1, weighted by
        // bin value
        const double class_mean2 =
            (mean - class_prob1 * class_mean1) / class_prob2;
