...
// rk: We define csi to be int to be backward compatible with older CSparse
// releases.
//     At some point we might adapt the internal structures of g2o to use the
//     same type as the one given is this header file.
#define csi int
...
G2O_CSPARSE_API cs *cs_spalloc(csi m, csi n, csi nzmax, csi values,
                               csi triplet);
...
G2O_CSPARSE_API cs *cs_permute(const cs *A, const csi *pinv, const csi *q,
                               csi values);
...
G2O_CSPARSE_API csi *cs_counts(const cs *A, const csi *parent, const csi *post,
                               csi ata);
G2O_CSPARSE_API double cs_cumsum(csi *p, csi *c, csi n);
G2O_CSPARSE_API csi cs_dfs(csi j, cs *G, csi top, csi *xi, csi *pstack,
                           const csi *pinv);
G2O_CSPARSE_API csi cs_ereach(const cs *A, csi k, const csi *parent, csi *s,
                              csi *w);
G2O_CSPARSE_API csi *cs_etree(const cs *A, csi ata);
G2O_CSPARSE_API csi cs_fkeep(cs *A, csi (*fkeep)(csi, csi, double, void *),
                             void *other);
G2O_CSPARSE_API double cs_house(double *x, double *beta, csi n);
G2O_CSPARSE_API csi cs_leaf(csi i, csi j, const csi *first, csi *maxfirst,
...
G2O_CSPARSE_API csi cs_reach(cs *G, const cs *B, csi k, csi *xi,
                             const csi *pinv);
G2O_CSPARSE_API csi cs_scatter(const cs *A, csi j, double beta, csi *w,
                               double *x, csi mark, cs *C, csi nz);
